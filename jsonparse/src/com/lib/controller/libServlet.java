...
public class libServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 만약 요청이 listdb라면?
			...
			String[] campinfoList = request.getParameterValues("lib");
			
			List<LibDto> dtos = new ArrayList<LibDto>();
			
			for(int i=0; i<campinfoList.length; i++) {
				String[] tmp = campinfoList[i].split("<");
				
				LibDto dto = new LibDto(tmp[0],tmp[1],tmp[2],tmp[3],tmp[4],tmp[5],tmp[6],tmp[7],tmp[8],tmp[9]);
